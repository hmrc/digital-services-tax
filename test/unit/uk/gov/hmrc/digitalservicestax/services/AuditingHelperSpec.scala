...
class AuditingHelperSpec extends AnyFreeSpec with ScalaCheckPropertyChecks with OptionValues {
  ...
  "AuditingHelper" - {
    "must build Return Submission Audit extended event" in {
      ...
      val extendedDataEvent: ExtendedDataEvent = AuditingHelper.buildReturnSubmissionAudit(DSTRegNumber("BCDST1234567890"), "", period, sampleReturn, true)
    ...
    "must build registration audit event" in {
      ...
      val extendedDataEvent: ExtendedDataEvent = AuditingHelper.buildRegistrationAudit(registration, "providerId", Some(FormBundleNumber("112233445566")), "SUCCESS")
    ...
    "must build callback audit event" in {
      ...
      val extendedDataEvent: ExtendedDataEvent = AuditingHelper.buildCallbackAudit(notification, FormBundleNumber("112233445566"), "SUCCESS", None)
