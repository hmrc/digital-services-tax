...
case class DesFinancialTransaction(
  chargeType: String,
...
case class DesFinancialTransactionResponse(
  idType: String,
  idNumber: String,
  regimeType: String,
...
object DesFinancialTransaction {
  ...
  implicit val financialTransactionFormatter = Json.format[DesFinancialTransaction]
  implicit val responseFormatter = Json.format[DesFinancialTransactionResponse]
  implicit val lineItemFormatter: Reads[List[data.FinancialTransaction]] = responseFormatter.map {
    case DesFinancialTransactionResponse(_, _, _, _, transactions) =>
      transactions.flatMap {
        case t: DesFinancialTransaction => t.items map { subItem => 
          ???
        }
      }
  }
