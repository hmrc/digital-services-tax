...
class DstRegChecker @Inject() (configuration: Configuration, db: MongoPersistence)(implicit ec: ExecutionContext)
    ...
    dstRegNumbers.foreach { dstRegNumber =>
      db.registrations.findByRegistrationNumber(dstRegNumber).foreach { optDstRegNum: Option[Registration] =>
        if (optDstRegNum.isEmpty) {
          logger.error(
            "\n<<>><<>><<>><<>><<>><<>><<>>ERROR NO REGISTRATION COULD BE FOUND FOR THE REGISTRATION NUMBER<<>><<>><<>><<>><<>><<>><<>>\n"
          )
        } else {
          logger.warn(
